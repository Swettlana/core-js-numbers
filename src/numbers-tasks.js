...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const lengthOne = Math.abs(x1 - x2);
  const lengthTwo = Math.abs(y1 - y2);

  return Math.sqrt(lengthOne ** 2 + lengthTwo ** 2);
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const absA = Math.abs(getDistanceBetweenPoints(0, 0, x1, y1));
  const abcB = Math.abs(getDistanceBetweenPoints(0, 0, x2, y2));
  return Math.acos(ab / (absA * abcB));
}
...
function isNumber(number) {
  if (Number.isFinite(number) && typeof number === 'number') return true;
  return false;
}
